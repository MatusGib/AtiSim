...
|---|---|---|
| **Parks, Wingrove, Bach & Mehta 1985**, J. Aircraft **22**(2), 124–129 | The entire vortex model: the Rankine field equations, `PARKS_CASES`' six numbers (r₀, V₀, spacing × 2 cases), the Scorer 2.7 spacing/diameter check, the ±25% α-inference caveat | **`unverifiable — source not available`.** AIAA paywalled; Unpaywall `is_oa:false`; NTRS holds metadata only. Bibliographic details confirmed via Crossref and match the code. **The headline result rests on this.** **Session 21 narrowed but did not close it:** Wingrove & Bach 1994 Fig. 4 independently confirms Morton at a 900 ft diameter — 450 ft of radius, `PARKS_CASES['morton']['r0']` to the digit — and independently *contradicts* Hannibal, giving 1000 ft of diameter against this file's 600 ft radius. Morton's agreement is now a second-source check on the radius convention; Hannibal's disagreement is an open conflict that only this document can settle. |
| **Wingrove & Bach 1994**, J. Aircraft **31**(4), 753–760 | `UPDRAFT_W0`, `UPDRAFT_SECONDS`, the Fig. 8 discriminator, the 5.2° pitch figure, the ±g asymmetry attribution | ~~**`unverifiable — source not available`.** Same reasons.~~ **SUPERSEDED session 21 — the paper was obtained and every claim above was read in it.** See the row below. |
| **Wingrove & Bach 1994**, J. Aircraft **31**(4), Jul–Aug 1994, 753–760 — **HELD, session 21** | as above, plus `WINGROVE_FIG4_CASES` and `WINGROVE_CASE_ALTITUDE` | **`verified`.** Six claims checked against the text, quoted location by location: `UPDRAFT_W0 = 80 ft/s` and `UPDRAFT_SECONDS = 20` — p. 756, *“a 20-s encounter with an 80 ft/s-updraft”*; the 5.2° figure — same sentence, *“resulted in a 5.2-deg variation in pitch angle”*; `+0.66 / −1.58 g` — p. 756, *“a positive load change of 0.66 g … a negative load change of 1.58 g”*; the 50/100 ft/s statement — p. 756, *“updrafts of 50 ft/s … can reach values of 100 ft/s”*; the Fig. 8 ordering — p. 758, *“In the extreme case there is only a change in pitch angle of 1.4 deg for a change in load of 1.99 g”*. **Two things the paper does NOT contain, both now recorded: it names no aircraft type anywhere, and its Fig. 4 gives Hannibal a 1000 ft core diameter where `PARKS_CASES` carries a 600 ft radius.** |
