...
| A12 | Thrust is `throttle × max_thrust × (ρ/ρ₀)^lapse`, ~~along body x, through the CG~~ along the aircraft's thrust line — body x through the CG by default; `boeing747` declares a 5.70 ft arm (CR-114494 revised) and 2.5° incidence (CR-2144 Table IX-3) since session 30 (ASSUMPTIONS C5) | `aero.thrust_force`, `aero.thrust_moment` | model form |
