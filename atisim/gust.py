...
That gap is why this file was written. `test_verification.py` checks the
integrator against a closed-form solution and measures RK4's observed order at
3.99982; nothing did the equivalent for the response to a gust. Every
turbulence figure in `docs/PROJECT.md` §4 was a comparison against a
*document*; none was a comparison against an *exact answer*. See
`docs/design/specs/2026-09-21-turbulence-response-validation-design.md`.

Four things live here, in the order the design's build order needs them:
...
every sign in between. Those are what V1 falsifies, and they are what carried
every turbulence number this project has published.
...
def mean_square_ratio(ac: Aircraft, V: float, H: float, spectrum,
                      *, n_points: int = 4000,
                      ...
                      wavelength_max: float = 40_000.0) -> float:
...
def realisation_mean_square_ratio(ac: Aircraft, V: float, H: float,
    ...
    **THAT IS WHY BOTH HALVES ARE RETURNED.** The realisation's own variance
    ...
    exactly. `scripts/gust_psd_identity.py` reports both, which is the only way
    to see which of the two any disagreement belongs to.
    """
...
def kussner_attenuation(omega: float, chord: float, V: float):
    ...
    Named for Kussner because the design phase is "bound the Kussner gust lag";
    ...
    **THE SIGN MATTERS MORE THAN THE SIZE.** The attenuation is a loss, so
    including it would REDUCE simulated load. `docs/PROJECT.md` §5 records the
    model falling short of a recorded encounter; this term therefore WIDENS
    that shortfall rather than explaining it, which is the direction that says
    it is not being raised for convenience.
    """
...
def measure_gust_transfer(ac: Aircraft, V: float, H: float, wavelength: float,
    ...
    inside `panel.ALPHA_LINEAR_DEG`. `scripts/gust_transfer_sweep.py
    --amplitudes` re-runs that sweep.

    **`stage_sampled` DEFAULTS TO TRUE HERE AND TO FALSE EVERYWHERE ELSE**, and
    the asymmetry is the point. Every other run in this project is compared
    against a document and keeps the frozen first-order default so its baseline
    does not move; this one is compared against an exact answer, so the
    integrator must not be what it measures. Running it with the hold instead
    is how phase V1 identified `PROJECT.md` §6(i): at the short period the held
    run reads +3.20%, +1.62%, +0.81% and +0.41% as dt is halved from 0.119 s,
    while the stage-sampled run sits at 0.0002% and does not move at all.
    Halving with dt is the wind hold's signature and not the response's.
    """
