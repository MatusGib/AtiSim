...
# altitude the loops hold. WGS84_J2 throughout: an autopilot is the aircraft's
# actual flight behaviour, not frame algebra.
...
def heights(hist):
    ...
    NOT `-pos_ned[:, 2]`. That is a tangent-plane height, and it is not a small
    distinction here: `sense` feeds the loop `state.altitude`, which is geodetic,
    and the ellipsoid falls away from the anchor's tangent plane as d^2/2R --
    3.9 m at the 7 km of ground track a 30 s run covers and 385 m at the 70 km a
    300 s one does. Read the tangent-plane height instead and the tests below
    would be asserting that the autopilot fails to hold an altitude it is in fact
    holding exactly.
    """
...
def test_disengagement_hands_back_the_current_deflections():
    ...
    targets = hold_targets()
    ap = ap_mod.engage(sense(state, ANCHOR), controls, targets, GAINS, AC)
    out, ap2 = ap_mod.autopilot(sense(state, ANCHOR), ap, targets, GAINS, AC, jnp.array(DT))
...
def test_heading_wrap_takes_the_short_way_round():
    ...
    roll = np.asarray(jax.vmap(quat_to_euler_ned, in_axes=(0, None))(hist, ANCHOR))[:, 0]
...
def test_anti_windup_recovers_promptly_from_an_unreachable_target():
    ...
    ap = ap_mod.engage(sense(state, ANCHOR), controls, unreachable, GAINS, AC)
    sim = integrate.init_sim(state, jax.random.PRNGKey(0))
    (sim, ap), _ = ap_mod.closed_loop_rollout(
        sim, ap, unreachable, GAINS, jnp.array(DT), AC, int(400.0 / DT), ANCHOR, EARTH
    )
    reached = float(geodetic_altitude(sim.state, ANCHOR))

    back = ap_mod.Targets(
        altitude=jnp.array(reached), heading=jnp.array(0.0), airspeed=jnp.array(V)
    )
    (_, _), (hist, _) = ap_mod.closed_loop_rollout(
        sim, ap, back, GAINS, jnp.array(DT), AC, int(300.0 / DT), ANCHOR, EARTH
    )
    altitude = heights(hist)
...
def test_closed_loop_rollout_vmaps_over_keys():
    ...
    (_, _), (hist, _) = jax.vmap(
        lambda s, a: ap_mod.closed_loop_rollout(
            s, a, targets, GAINS, jnp.array(DT), AC, 500, ANCHOR, EARTH
        )
    )(sims, aps)
    assert hist.pos_ecef.shape == (4, 500, 3)
    assert np.asarray(hist.pos_ecef).std(axis=0).max() == pytest.approx(0.0, abs=1e-12)
