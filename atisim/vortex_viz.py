...
def fly_from_state(
    ...
    *** `stage_sampled` AND WHY IT DEFAULTS TO FALSE. ***
    ...
    makes the scheme FIRST ORDER through a spatially varying field (assumption
    E4: observed order 1.05 held against 4.06 re-sampled). `wind.field_model`
    MARKS its output as safe to re-evaluate, but `step` gates on its own
    `stage_sampled` ARGUMENT, and this function passes it through. At the short
    period a held run through a single gust sinusoid disagrees with the exact
    transfer function by +3.20%, +1.62%, +0.81% and +0.41% as dt is halved from
...
# ---------------------------------------------------------------------------
# TM-102186 Fig. 8 -- NOT Wingrove & Bach's Fig. 8 above. Six aircraft through
# Mehta's field: the three-aircraft ordering, and the mechanism behind it.
# Moved here from scripts/cat_validation.py so that test_cat_validation.py can
# assert what the script prints and the validation notebook shows.
# ---------------------------------------------------------------------------
