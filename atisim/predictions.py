...
PREDICTIONS: tuple[Prediction, ...] = (
    ...
    Prediction(
        ...
        status="SETTLED",
        outcome=(
            "RIGHT ON ITS CLAIM AND WRONG ON ITS MECHANISM, and it is only "
            "answerable at all because the hazard it made a precondition "
            "turned out to be resolvable. Settled by "
            "scripts/tpaws_peak_factor.py, N = 48 per arm, the same flights as "
            "the V5 audit -- its whole-record column reproduces that audit's "
            "3.8674 +/- 0.0450 and 4.0199 +/- 0.0611 to four decimals, which "
            "is what licenses reducing them a second way. "
            "THE DEFINITION, READ OFF THE DOCUMENT AS THE SEAL REQUIRED: "
            "NASA/TM-2012-217337 printed p. 5 defines sigma_dn over a sliding "
            "5 s window, subtracting the mean over that same window -- a "
            "running STANDARD DEVIATION, not a running RMS about the record "
            "mean; printed p. 125 names the tabulated scalar 'the peak "
            "sigma_dn'; printed p. 8's column group header, 'Peak In Situ "
            "Turbulence', governs the extremes alongside it. So TPAWS' "
            "denominator is the MAXIMUM OVER THE ENCOUNTER of that running "
            "quantity and its numerator is the encounter's extreme. "
            "THE THRESHOLD IS RE-DERIVED AND MOVES: 2.3831 with population sd "
            "0.4942, over the 53 rows the table actually has, not the 2.386 / "
            "0.506 this seal transcribed from a 51-row parse that had silently "
            "dropped 232-05 and 235-05, the two rows whose altitude prints as "
            "a range. That correction is 0.13% and decides nothing by itself. "
            "WHAT DECIDES IT IS THE WINDOW. On V5's whole-record sigma the "
            "model reads 3.8674 and the prediction would be WRONG by a wide "
            "margin. On TPAWS' own definition it reads 1.9421 +/- 0.0246 -- "
            "BELOW 2.3831 by 17.9 standard errors, and 0.89 population sd "
            "below the mean, so inside the design's S2 band as well as under "
            "its centre. The verdict does not turn on the one DECLARED "
            "quantity either: chopping the record into encounter-length "
            "segments to match the document's 'several seconds to a minute' "
            "gives 1.9909, 1.9391 and 1.9236 at L = 15, 30 and 60 s, all "
            "below. "
            "THE MECHANISM IS NOT THE ONE ARGUED ABOVE, and that is the part "
            "worth carrying forward. The reasoning bet that the Shinozuka "
            "construction's peak-poorness would beat the narrowband "
            "resonance. Under this reduction the construction contributes "
            "essentially nothing: Shinozuka minus Gaussian is +0.0092 +/- "
            "0.0375, 0.25 se, and the SIGN IS REVERSED from the -0.1525 "
            "(2.01 se) that the_shinozuka_realisation_is_peak_poor was "
            "settled on. That companion result was measured on a whole-record "
            "sigma and does not survive a 5 s running one. Neither prediction "
            "is edited on the strength of that -- it is recorded in "
            "PROJECT.md section 4 beside both. What put the model below TPAWS "
            "is the sigma window, not the construction. "
            "WHAT THIS DOES NOT LICENSE: reading 1.9421 as the model agreeing "
            "with TPAWS about anything physical. A second mismatch is not "
            "removed by any reduction here -- TPAWS' encounters are convective "
            "and last under a minute, this project's records are 1200 s of "
            "stationary Dryden -- and section 5's altitude objection stands: "
            "three of the 53 rows are in band for boeing747 and a fourth "
            "straddles the floor. PROJECT.md section 4, phase S2."
        ),
    ),
)
