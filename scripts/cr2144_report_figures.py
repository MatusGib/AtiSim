...
from the numbers PROJECT.md section 4 quotes:
  ...
  s2  the four longitudinal modes against Table IX-5, before and after
...
def main():
    ...
    after = cm.errors_vs_ix5(rows["SHIPPED boeing747, FC9 set declared"])
    ...
    for off, vals, color, lab in ((0.19, before, C_BEFORE, "Before (no speed derivatives)"),
                                  (-0.19, after, C_ATISIM, "After (speed derivatives declared)")):
        v = [100 * vals[k] for k, _ in names]
        ax.barh(y + off, v, height=0.34, color=color, label=lab)
        for yy, val in zip(y + off, v):
            ax.text(val, yy, f" {val:+.1f}% " if val >= 0 else f" {val:+.1f}% ", va="center",
                    ha="left" if val >= 0 else "right", color=INK2, fontsize=8)
    ...
    for ac, color, lab in ((sd.BARE, C_BEFORE, "Before (no speed derivatives)"),
                           (sd.AC, C_ATISIM, "After (speed derivatives declared)")):
        r = fly(ac, field)
