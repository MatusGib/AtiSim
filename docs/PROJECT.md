...
| **State** | **Based on the pre-PR-#3 `origin/main` (`4e5191e`), so it is BEHIND the session-30 merge and will conflict with it in §0, §4 and §9. Not pushed.** `scripts/sensitivity_mass_diagnosis.py` plus this document: §0, §4's first entry, in-place notes on session 29's `mass` claims, and §9. No model or test code changed, so the suite was not run |
...
- **NOT found: registration, variant (−10 or −30) and gross weight.**
