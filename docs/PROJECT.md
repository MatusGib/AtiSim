...
| **Status** | **TRACKED, NOT REVIEWED, NOT USED.** No number in it has been checked against Table IX-4 and none reaches any aircraft entry |
...
**926 passed, 1 skipped, 1 xfailed in 1,566 s**, and the solver notebook's 13 cells green.
...
| `.venv/Scripts/python.exe -m pytest --nbval-lax notebooks/ -q` | **The second gate.** Executes ~~`notebooks/solver-validation.ipynb`~~ both notebooks — `solver-validation.ipynb` and, since session 32, `validation-ladder.ipynb`, which walks §1's claim — so they cannot rot: **24 passed, 157 s**. CI runs it after the suite (`.github/workflows/tests.yml`). Needs the `dev` extra (`jupyter`, `nbval`). Deliberately *not* in `testpaths` and `--nbval-lax` is deliberately *not* in `addopts`: that would make every `pytest` run fail with "unrecognized arguments" wherever nbval is absent. **Run it from a worktree with an ABSOLUTE `PYTHONPATH`** — nbval starts the kernel with its cwd in `notebooks/`, so a relative `PYTHONPATH=.` resolves to the wrong directory and `atisim` silently loads from the main checkout. |
