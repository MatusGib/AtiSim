...
- **`AGENTS.md` is new**, and rule 1 is *"nothing is done until it is in `docs/PROJECT.md`"*. Two digitisations and a full LES run had already been done in another worktree -- untracked, outputs gitignored -- while that worktree's `PROJECT.md` listed them as open work. Five stranded scripts are now tracked.
